...
- The realistic trigger is a manual seed racing the cron (retention delete of run B removes run A's not-yet-flipped sets â†’ `not found` raise â†’ abort path).
- With the fix, both orderings of the cleanup-vs-flip race converge safely under READ COMMITTED.

## Proposed Solutions

1. **(Recommended, one line)** Filter the cleanup delete: `.delete().eq("is_active", false).in("id", ids)` â€” a flipped set is complete, valid, live data; only inactive debris is removed. Also log cleanup errors instead of swallowing.
2. Single flip call (no chunking) â€” caps at PostgREST payload limits, loses batching benefits.
